...
- Copy the config.py file in Nancy>config folder
- Make a new python environment
    If you are using anaconda just type ```conda create -n Nancy python==3.8.5 ``` in anaconda prompt
- To activate the environment ``` conda activate Nancy ```
...
## Code Structure
    ...
    ├── driver
    ├── Nancy              # Main folder for features 
    │   ├── config          # Contains all secret API Keys
    │   ├── features        # All functionalities of Nancy 
    │   └── utils           # GUI images
    ├── __init__.py         # Definition of feature's functions
    ├── gui.ui              # GUI file (in .ui format)
    ├── main.py             # main driver program of Nancy
